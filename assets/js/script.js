    ...
    { // Секция валидации
        ...
        { // Группа функций для валидации полей при отправке формы
            ...
            $('.search-filter').on('submit', function () {
                // Если поле пусто - пропускаем
                if ($(this).find('.search-filter__input[name="price_from"]').val() === '') {
                    $(this).find('.search-filter__input[name="price_from"]').prop('disabled', true);
                }
                if ($(this).find('.search-filter__input[name="price_to"]').val() === '') {
                    $(this).find('.search-filter__input[name="price_to"]').prop('disabled', true);
                }
            });
